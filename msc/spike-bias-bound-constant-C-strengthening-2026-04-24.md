...
**Pressure point.** The Class-2 bias bound in `#result-section-ii-survival` (line 109) and its refined form in `#scope-ambiguity-modulation` (line 43):
...
**M1 — Preferred: new appendix segment `#deriv-bias-bound`.** House the full derivation (Attempts A + B + E) in an appendix segment with:
- *type:* `derivation`
...
**M2 — Update `#result-section-ii-survival` and `#scope-ambiguity-modulation`.** In both segments, replace the Working-Notes line *"Without the constant, the bound is order-of-magnitude guidance, not a theorem"* with a cross-reference to `#deriv-bias-bound`. Upgrade the bias-bound statement's epistemic tier from *order-of-magnitude* to *conditional (exact under H1-H3 or H1+H4)*.
...
**M4 — Shared KL-to-displacement lemma.** The Pinsker / Talagrand-T2 / LSI machinery is now used in two places: `#deriv-variational-sector-condition` (KL → sector-constant degradation) and the proposed `#deriv-bias-bound` (KL → posterior displacement). Factoring the shared machinery into a *KL-to-state-distance template* (analogous to `#result-sector-persistence-template`) would be a clean structural move. This is a portfolio-level proposal worth adding to `msc/architectural-proposals-2026-04-22.md`.
...
| Candidate shared KL-to-displacement template | **Discussion-grade** | Observation about structural sharing between `#deriv-variational-sector-condition` and proposed `#deriv-bias-bound` |
