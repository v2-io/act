...
**Companion material:** `msc/spike-L1-worked-example.md` (strict-vs-soft handling at line ~276); `01-aad-core/src/example-L1.md` (already-published L1' treatment, lines 120–136); `msc/SPIKES.md` line 43.
...
The "formal transfer ... is open" status for L1' is honest — it is exactly what `#deriv-strategic-dynamics` line 513 says. No promotion of L1' beyond what the spike material supports.
...
| `#deriv-strategic-dynamics` | Prop B.6 (line 278), L0 comparison (327), summary table (343) | No | **Correct as-is.** Prop B.6 is explicitly the strict-prerequisite case (uses $\theta_{i \mid \neg C} = 0$). Already names L1' as the open extension at line 513. |
...
This is consistent with the spike material (`msc/spike-L1-worked-example.md` does not derive L1' formally; `01-aad-core/src/example-L1.md` line 134 explicitly notes the open status).
...
6. **Naming: L1' vs L1.5.** `msc/SPIKES.md` line 43 says "L1.5 / L1' soft-facilitator case flagged as gap". The published segments (`#example-L1`, `#deriv-strategic-dynamics`, `#disc-approximation-tiering`) all use L1'. This spike sticks with L1'. Confirm SPIKES.md line 43 should be updated to drop "L1.5" at next pass.
