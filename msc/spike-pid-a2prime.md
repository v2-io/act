...
- The six exclusions in §10 are real; A2' genuinely does not derive for (a) RHP-zero-dominated plants outside a small operating region, (b) uncontrolled time-varying plants, (c) high-dim stochastic plants without state estimation, (d) hard nonlinearities exceeding Popov bound, (e) deadtime-dominant plants without delay compensation, (f) coupled MIMO-PID without decoupling. The segment should enumerate these visibly — per `#identifiability-floor`'s pattern of treating scope-exits as architectural, not as caveats.
