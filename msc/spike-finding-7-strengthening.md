...
| `#def-system-coherence` $E[\text{proximity}(\text{changes within } m)]$ | Mean inverse `#def-change-distance` between intra-module changes per commit | Yes — change locations within a commit's diff are exact; the proximity is then a function of file paths and line ranges in the diff. | **EXACT** |
...
| `#def-change-distance` $d(c_i, c_j)$ | File-path traversal + line offsets within diffs | Yes — distance hierarchy (lexical / file / module / service) is a function of the file paths and line ranges in commit diffs. | **EXACT** |
