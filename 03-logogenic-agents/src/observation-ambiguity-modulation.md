...
**The scope condition $\kappa_{\text{eff}} = \kappa \cdot \mathcal{A}$ and the bias bound** are *conditional*. The multiplicative composition is the simplest form consistent with the two boundary conditions ($\mathcal{A} = 0 \Rightarrow$ no bias; $\kappa = 0 \Rightarrow$ no bias) and is structurally motivated by the survival analysis (`msc/spike-coupled-survival-analysis.md`), but it is not derived from a specific coupled-update model. The actual composition could be nonlinear ($\kappa^a \cdot \mathcal{A}^b$ with $a, b \neq 1$), or involve threshold effects, or depend on features of the observation beyond $\mathcal{A}$ alone. The constant $C$ in the bias bound is domain-dependent and not computed by the theory.
...
Max attainable: the multiplicative composition could be derived from the information geometry of the coupled update, relating $\mathcal{A}(e_\tau)$ and $\kappa_{\text{processing}}$ to the Fisher-information decomposition of the likelihood $P(o \mid M, G)$. A specific model of how goal-conditioning enters the likelihood function (e.g., attention-weighted Bayesian inference) would close this gap — feasible in principle but not attempted here.
