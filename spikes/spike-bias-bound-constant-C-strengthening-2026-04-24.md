...
**M2 — Update `#result-section-ii-survival` and `#scope-observation-ambiguity-modulation`.** In both segments, replace the Working-Notes line *"Without the constant, the bound is order-of-magnitude guidance, not a theorem"* with a cross-reference to `#deriv-bias-bound`. Upgrade the bias-bound statement's epistemic tier from *order-of-magnitude* to *conditional (exact under H1-H3 or H1+H4)*.
