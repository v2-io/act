...
**What directed separation buys the theory.** When it holds, all of Section I carries over unchanged — the adaptive machinery operates on $M_t$ without needing to know whether the agent has purpose. $f_M$, $\delta_{\text{epistemic}}$, $\eta^*$, $\mathcal{T}$, the persistence condition — all remain valid as stated. Without directed separation, every Section I result would need to be re-derived with $G_t$ coupling terms, and the theory's modular structure would collapse.
