...
- The bridge lemma's contraction assumption (sector-bounded correction implies contraction of the full update map) is verified for Kalman-type agents but remains an independent assumption for general agents. Proving it from (A4) alone — or identifying the minimal additional conditions needed — is the key open step for promoting the bridge lemma from conditional to derived. The discrete-time linear recurrence argument is itself standard (Elaydi 2005); the gap is in the contraction premise, not the recurrence.
