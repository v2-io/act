...
- `CLAUDE-2.md` — **Deep architectural detail** (recent cycles, settled commitments, novel results, convergent choices). **Read only when Joseph indicates the task is not a de-novo audit.**
...
**Supporting:**
- `bin/` — Build, lint, and slug tools (`align-slug`, `build`, `build-tex`, `lint-md`, `lint-outline`, `md2context`, `naming-aggregate.rb`, `rename-slug`)
