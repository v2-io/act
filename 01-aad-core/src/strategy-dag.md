...
All L0 formal results transfer to correctly constructed L1 DAGs *for strict-prerequisite common causes* because the augmented DAG is a standard AND/OR DAG that satisfies causal sufficiency. For L1' (mixture form) the formal results transfer through each sub-plan separately with an additional weighted-combination layer; this has not been formally derived and is an open item. The practical challenges are: (1) identifying which common causes matter enough to model explicitly, (2) classifying each identified common cause as strict or soft, (3) restructuring the DAG correctly for each. All three are modeling judgments, not mechanical procedures.
