...
**Why a DAG.** The DAG structure is not a modeling convenience but a *consequence* of operational requirements on any causally-reasoning bounded agent. #graph-structure-uniqueness derives this in the tradition of Cox's theorem: directed temporal order plus probabilistic uncertainty plus causal sufficiency force the representation to be a causal Bayesian network with the Markov factorization property. Acyclicity is proved from temporal ordering over a finite horizon. What remains a formulation choice is the *parameterization within* the DAG structure: AND/OR combination with single-parameter edges is the AAD choice, motivated by parsimony and convergence across three independent formalism attempts, but alternative parameterizations (within the proved graphical structure) are legitimate research directions.
