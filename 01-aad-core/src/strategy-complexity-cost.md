...
Under deterministic $\pi^\ast$, $D_{\mathrm{KL}}(\pi^\ast \Vert Q_{\Sigma_t}) = -\log Q_{\Sigma_t}(a^\ast)$ — finite and graded whenever $Q_{\Sigma_t}(a^\ast) \gt 0$. The opposite-direction $D_{\mathrm{KL}}(Q_{\Sigma_t} \Vert \pi^\ast)$ equals $+\infty$ whenever $Q_{\Sigma_t}$ places any mass off $a^\ast$, giving a vacuous bound. The regret-bound argument therefore **forces the KL direction** with $\pi^\ast$ first. (The specific divergence within the admissible family — TV, reverse-KL, Bretagnolle-Huber, $\chi^2$, Rényi-$\alpha$ — is a canonical choice; reverse-KL is selected on gradient-tractability, variational-inference-alignment, Fisher-geometry, and MDL-interpretation grounds. See #strategy-cost-regret-bound §6 for the admissible-divergence family characterization and §7 for the linear-vs-square-root $\beta_\Sigma$ trade-off.)
...
The description length formulation is a *formulation* --- it applies standard MDL to the strategy DAG, which is a representational choice not a derived necessity. The IB objective is *formulation (strengthened by regret bound)* in its variational form (above): the specific KL *direction* ($\pi^\ast$-first, i.e., reverse-KL in the variational-inference vocabulary) is *derived* as an upper regret bound via Pinsker's inequality under bounded value range and deterministic $\pi^\ast$ (see Regret-bound derivation paragraph above; the full derivation, admissible-divergence family analysis, tightness comparison, and $\beta_\Sigma$ linear-vs-square-root trade-off are in the appendix #strategy-cost-regret-bound). The specific choice of reverse-KL within the admissible family of regret-bounding divergences (TV, reverse-KL, Bretagnolle-Huber, $\chi^2$, Rényi-$\alpha$) is a *canonical formulation* selected on four grounds: gradient-tractability, alignment with variational inference (Friston et al. 2017; Da Costa et al. 2020; Parr & Pezzulo 2022), Fisher-information geometry (Amari & Nagaoka 2000), and MDL coding interpretation. None of these is a uniqueness argument — reverse-KL is canonical, not unique. The regret-bound derivation closes the direction ambiguity that the earlier V-medium move (commit `a14682e`) left open: the initial V-medium form used $D_{\mathrm{KL}}(Q_{\Sigma_t} \Vert \pi^\ast)$ (forward-KL), which is $+\infty$ under deterministic $\pi^\ast$ whenever $Q_{\Sigma_t}$ has any off-optimum mass — a different-valued but structurally identical degeneracy to the Shannon-MI zero it replaced. The $\pi^\ast$-first direction escapes both degeneracies by construction.
