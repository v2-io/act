...
2. **Strategic / adversarial equilibria.** Two-agent strategic dynamics have saddle-point equilibria of the dynamics even when Nash-unique. Contraction analysis requires attracting fixed points; saddle points break the framework. **Contraction metrics cover the cooperative half of Section III** (`#team-persistence`, cooperative `#critical-mass-composition`, cooperative `#composition-closure`). The adversarial / strategic half belongs to `#strategic-composition` and `#adversarial-destabilization`, which require equilibrium-theoretic rather than Lyapunov-contraction machinery.
