...
AAD consistently runs a three-part epistemic posture across state spaces that admit no tractable exact treatment in general: name the **separable core** where identification is clean, name the **structured repair** that recovers identification under explicitly-added machinery, and name the **general open** case where the problem is either intractable or structurally unidentifiable. Six ladders in the current theory share this shape. This segment names the pattern as an organizing principle, catalogs the instances, and makes the complementarity with `#identifiability-floor` (which names the *negative half* of AAD's scope) explicit.
...
| **Scope hierarchy** (#agent-spectrum) | **Adaptive** — basic feedback loop; #scope-condition satisfied | **Agency** — goal-bearing with $\lvert\mathcal{A}\rvert \geq 2$ and at least one action with causal effect; adds $O_t$ and $\Sigma_t$ machinery (Section II) | **Composite** — multi-agent / team / logogenic; Section III gaps listed at the OUTLINE level (latent structural diversity, endogenous coupling, composition transition dynamics, agent opacity) |

Each row independently satisfies the three-part shape. The shared shape is not designed-in: it arises because AAD faces six distinct sources of intractability and applies the same posture (name the clean case, name the repair, name what remains open) to each.
...
The `#identifiability-floor` names the *negative half*: structural no-go results (impossibility under limited information) that characterize why specific general-open cases remain open. Two floors are currently derived:
...
- **#separability-pattern** (this) — what each tier positively commits to (separable core / structured repair / general open).
...
**The A2' sub-scope partition is a binary special case.** The A2' strengthening spike (msc/spike-a2-prime-strengthening.md, commit `80b40d2`) partitioned AAD agents into sub-scope α (A2' derived under #gain-sector-bridge directional fidelity) and sub-scope β (A2' assumed). This is binary rather than three-part, but shares the separable-core/general-open shape: α is the separable core (derivation succeeds), β is the general case (assumption required). It is not currently counted as a seventh ladder because it lacks the intermediate structured-repair tier — but if a future move identifies a mid-scope where A2' is recoverable under additional machinery (e.g., converse Lyapunov + metric-specific repair), the partition could extend to three parts.
...
- **Cross-ladder monotonicity.** The six ladders interact — an agent operating at L0 correlation + C2 convention + Class 1 architecture + Tier 1 contraction + Regime A identification + Agency scope is in a specific combined regime. Does cross-ladder monotonicity hold in any direction? E.g., does moving from L0 to L1 change anything about the Convention hierarchy's guarantees, or do the ladders factor independently? `#approximation-tiering`'s Working Notes flag this as an open question; the separability pattern's complementarity with the identifiability floor adds a further axis (which combinations are known-unidentifiable per #identifiability-floor).
