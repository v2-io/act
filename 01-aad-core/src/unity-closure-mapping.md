...
**Connection to the Information Bottleneck ( #information-bottleneck).** The rate-distortion shape is not coincidental. Projection admissibility condition (P1) in #composition-closure is structurally an IB relevance-preservation constraint: the projection must retain at least $(1 - \epsilon_I)$ of the predictive mutual information about next observation given action. This places admissible projections on (or near) the IB frontier for the relevant rate-distortion problem. Unity dimensions — measured as mutual-information-like quantities between sub-agent state components — parametrize the frontier's shape. This suggests a larger unification conjecture: the three admissibility conditions (P1)-(P3) in #composition-closure may reduce to "projection attains the IB frontier at the implicit rate set by $\dim \mathcal X_c$" plus the Lipschitz regularity (P2). If the conjecture holds, several currently-separate constructions (admissibility, shared intent in #shared-intent, model compression via #information-bottleneck, strategy cost in #strategy-complexity-cost) would unify as IB instances with different relevance variables. Caveats: (P2) is not naturally IB and must remain separate; the IB frontier depends on the relevance variable, so the theory would need to fix it unambiguously; Gaussian IB is fully tractable but general IB is not, limiting the formal reach of the unification to cases where closed forms exist.
...
- **Relationship to #composition-scope-condition.** This segment describes quality *conditional* on composition existing (the $U_O$ scope threshold in #composition-scope-condition passes). The rate-distortion curves parametrize quality; they do not address whether a composite exists at all. For multi-agent systems below the scope threshold, closure-defect quality talk is a category error — there is no composite whose closure defect to measure.
