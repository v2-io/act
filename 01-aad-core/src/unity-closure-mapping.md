...
Full derivation and numerical validation: `msc/spike-unity-closure-mapping.md`.
...
- **Mori-Zwanzig cross-check.** The Mori-Zwanzig spike (`msc/spike-mori-zwanzig-composition.md`) identifies the non-degenerate Kalman case as exercising zero-lag kernel $K_0$ non-trivially, with $\lVert K_0 \rVert$ scaling with $\lvert\Delta K\rvert$. This is consistent with the two-axis finding here — MZ and rate-distortion should give the same lower bound quantitatively. Formal equivalence not yet established.
- **Relationship to #composition-scope-condition (proposed).** Under the scope-condition framing for composition (see `msc/spike-symbiogenic-composition.md`), this segment describes quality *conditional* on composition existing ($U_O \geq \epsilon_{\text{comp}}$ gate passes). The rate-distortion curves parametrize quality; they do not address whether a composite exists at all. Once #composition-scope-condition is promoted, this segment's Discussion should note the conditional presupposition.
