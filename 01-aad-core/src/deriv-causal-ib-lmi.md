...
- 2026-04 (*nominally comprehensive on the exploration-source contrast*, via `msc/Novelty_defense_and_integration.md` Pillar 1): The Undermind report on Pillar 1 ("Causal insufficiency and forced exploration") established the active-inference EFE framing as the canonical alternative source for exploration drives. This finding's matrix-Lagrangian LMI is the directional sharpening of the scalar derivation in `#deriv-causal-ib-exploration` and inherits that segment's differentiation posture against EFE-style epistemic-value accounts.
