...
- **Partial: Mori-Zwanzig connection.** Worked in `msc/spike-mori-zwanzig-composition.md` (2026-04-20). *What closes*: under stationarity and coordinate-compatibility, the MZ-optimal Markovian macro-dynamics $f_c^{\text{MZ}} = P_\Lambda U P_\Lambda$ (conditional expectation given the macro-state) provides a concrete benchmark, and $\varepsilon^\ast \geq \lVert Q_\Lambda U P_\Lambda \rVert_{\text{op}}$ — a zero-lag kernel bound — when $f_c^{\text{MZ}} \notin \mathcal M_{\text{adm}}$. The bridge-lemma contraction corresponds conceptually to spectral gap of $Q_\Lambda U$. *What does not close*: the full-kernel bound $\varepsilon^\ast \geq C \cdot \lVert K \rVert_{\ell^1}$ is a type mismatch — $\varepsilon^\ast$ is per-step, $\lVert K \rVert_{\ell^1}$ is trajectory-accumulation. Natural home for the full-kernel norm is the bridge lemma's trajectory-error bound, not $\varepsilon^\ast$ directly. *Hard obstruction*: MZ's stationarity assumption fails for purposeful agents with non-stationary auxiliary state (Beta-Bernoulli with diverging $n$) — exactly the cases where $\varepsilon^\ast > 0$ genuinely. Extending to these requires innovation-frame reformulation, not attempted. See spike for the full argument and four specific obstructions.
- **Open: Information Bottleneck unification.** Seeded in `msc/spike-unity-closure-mapping.md` §6 (2026-04-20). (P1) is structurally an IB relevance-preservation constraint with relevance variable "next observation given action." (P1)+(P3) together may be equivalent to "projection attains the IB frontier at the implicit rate set by $\dim \mathcal X_c$"; (P2) remains a separate regularity requirement. Unity dimensions ( #unity-dimensions) may index the IB rate-distortion function. If this holds, several currently-separate constructions (admissibility, shared intent, model compression, strategy cost) are IB instances with different relevance variables. Not attempted in full; see spike §6.4 for the conjecture and §6.2 for why caveats (Gaussian-only closed forms, relevance-variable specification) block a trivial derivation.
- **Two independent drivers of $\varepsilon_x$ identified.** The non-degenerate Kalman test case (`msc/spike-unity-closure-mapping.md` §10, 2026-04-20) shows $\varepsilon_x$ depends on *both* sub-agent unity (captured by $U_M$) *and* update-rule heterogeneity ($\Delta K$ in the Kalman case). $\Delta K = 0$ gives $\varepsilon_x = 0$ at every correlation; $\Delta K \neq 0$ gives $\varepsilon_x > 0$ even at perfect correlation. Heterogeneity is not captured by the four unity dimensions in `#unity-dimensions` as currently defined. This is a gap in that segment, not in this one — the closure defect correctly registers both drivers.
