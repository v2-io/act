...
where $\hat o_{c,t}$ is the macro-model's prediction of the next macro-observation. This ensures the macro-agent can detect prediction errors — the foundation of adaptation ( #mismatch-signal).
...
- **Open: Information Bottleneck unification.** (P1) is structurally an IB relevance-preservation constraint with relevance variable "next observation given action." (P1)+(P3) together may be equivalent to "projection attains the IB frontier at the implicit rate set by $\dim \mathcal X_c$"; (P2) remains a separate regularity requirement. Unity dimensions ( #unity-dimensions) index the IB rate-distortion function ( #unity-closure-mapping). If this conjecture holds, several currently-separate constructions (admissibility, #shared-intent, model compression via #information-bottleneck, strategy cost in #strategy-complexity-cost) unify as IB instances with different relevance variables. Caveats blocking a trivial derivation: Gaussian IB is tractable in closed form but general IB is not; the IB frontier depends on the relevance variable, so the theory must fix it unambiguously; (P2) Lipschitz regularity is not naturally IB.
