...
**The (PI)/Čencov observation.** Two metric-α₂ cases (information-metric Kalman, Fisher-metric exp-family) are AAD-internally forced under the (PI) axiom in `#scope-agent-identity` combined with Čencov's 1982 uniqueness theorem — the fourth primary instance of the meta-pattern in `#disc-additive-coordinate-forcing`. The remaining three metric-α₂ cases (Hessian, Lyapunov-linear-Hurwitz, Lyapunov-PID) are theorem-imported: the metric choice is problem-specific (matched to loss curvature or plant structure) rather than forced by an AAD-internal axiom.
