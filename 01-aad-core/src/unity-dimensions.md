...
- **$U_O$ as scope vs. quality.** Per `msc/spike-symbiogenic-composition.md` (2026-04-20), $U_O$ may have a dual role: existence condition for composition (does a composite exist at all?) and quality metric (given existence, how well does action compression work?). If promoted, this segment would separate the two roles, with $U_O$ as scope living in a proposed #composition-scope-condition segment and the quality-metric role remaining here. Decision deferred.
