...
- **Update-rule heterogeneity is a missing axis** (identified 2026-04-20, `msc/spike-unity-closure-mapping.md` §10). The four unity dimensions measure shared *content* (information, goals, policies, observations). In the non-degenerate Kalman case, different Kalman gains $K_1^\ast \neq K_2^\ast$ across agents produce $\varepsilon_x > 0$ independent of any unity dimension — even at perfect process correlation. Candidate resolutions: (A) add a fifth dimension "update homogeneity" $U_f$, (B) reinterpret $U_\Sigma$ broadly to cover $f_M$ heterogeneity in passive-estimator cases, or (C) accept that the closure defect has a two-axis structure (unity × homogeneity) rather than a single unity axis. Option (C) is the honest minimal framing; (A) and (B) are more theoretically ambitious. Decision deferred pending segment promotion of `#unity-closure-mapping`.
- **Joint $(U_O, U_\Sigma) \to \varepsilon_a$ dependence** resolves the four-unity-three-closure correspondence (the table above now reflects this): state error tracks $U_M$, action error tracks *both* $U_O$ (target alignment) *and* $U_\Sigma$ (policy alignment), observation error tracks $U_{\text{obs}}$. The two dimensions jointly controlling action error are physically distinct: $U_O$ is about evaluation/preference agreement; $U_\Sigma$ is about execution-path agreement. Agents with identical objectives but different execution plans have high $U_O$, low $U_\Sigma$; agents coordinating on arbitrary shared routines have high $U_\Sigma$, low $U_O$.
- **Reframing status.** After the 2026-04-20 spike, this segment's quantitative claims now depend on the companion (proposed) segment `#unity-closure-mapping`. This segment remains the canonical location for the definitions and qualitative interpretation of unity; quantitative mappings to closure defect live in the companion.
