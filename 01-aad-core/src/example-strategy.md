...
- This example does not exercise strategy-level tempo ( #schema-strategy-persistence) or the three-way tradeoff ( #disc-exploit-explore-deliberate). A future worked example targeting these would need a system with genuine deliberation costs — simulation in `msc/sim-three-way-tradeoff.py` provides a drifting-bandit version but found deliberation rarely dominates in simple settings.
