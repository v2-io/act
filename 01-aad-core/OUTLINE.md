...
| A   | Discussion |     | [#identifiability-floor](src/identifiability-floor.md)                 | Meta-pattern: structural no-go results from external info-theoretic theorems (CHT, Cramér-Rao, Liberzon common-Lyapunov-nonexistence); three instances (on-policy L0-detection, L1' mixture-identifiability, composite-contraction-from-component-data) | draft |
| A   | Discussion |     | [#separability-pattern](src/separability-pattern.md)                   | Meta-pattern: separable-core / structured-repair / general-open across seven ladders (correlation, convention, architecture, contraction, identification, scope, A2'-scope via #contraction-template); positive-half complement to #identifiability-floor | draft |
| A   | Discussion |     | [#additive-coordinate-forcing](src/additive-coordinate-forcing.md)     | Meta-pattern: AAD forces coordinates via uniqueness theorems on AAD-internally-motivated axioms at four layers (chain-log-additive-identity; divergence-reverse-KL and update-log-odds via Cauchy-FE; metric-Fisher via (PI)/Čencov-invariance); 1-anchor-plus-3-theorem structure with Lyapunov and IB Lagrangian as adjacent family members | draft |
